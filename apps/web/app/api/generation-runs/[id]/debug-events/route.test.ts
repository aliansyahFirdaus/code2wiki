...
describe("generation debug events route", () => {
  it("passes polling params and returns ordered events response", async () => {
    ...
    const response = await GET(new Request("http://test.local/api/generation-runs/run-1/debug-events?afterId=event-1&limit=50"), {
      params: Promise.resolve({ id: "run-1" })
    });

    expect(response.status).toBe(200);
    expect(await response.json()).toEqual({ events: [{ id: "event-2" }], nextAfterId: "event-2", summary: { taskCounts: { queued: 1 } } });
    expect(mocks.loadGenerationDebugEvents).toHaveBeenCalledWith({ generationRunId: "run-1", afterId: "event-1", since: null, limit: 50 });
  });
