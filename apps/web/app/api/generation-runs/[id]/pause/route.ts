...
export async function POST(_request: Request, context: Context) {
  ...
  try {
    const [run] = await getDb()
      ...
      .set({ controlState: "PAUSED", advanceRequestedAt: null })
    ...
    return NextResponse.json({ error: { code: "GENERATION_RUN_PAUSE_FAILED", message: sanitizeErrorText(error) } }, { status: 503 });
  }
}
